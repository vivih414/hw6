...
// Almost Complete - Fill in the if statement below. (Did)
template <typename KeyType>
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), totalProbes_(0), mIndex_(0), resizeAlpha_(resizeAlpha), size_(0)
{
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
    for(typename std::vector<HashItem*>::const_iterator it = table_.begin(); it != table_.end(); it++)
    {
        if(*it != nullptr)
        {
            delete *it;
...
{
    double load = static_cast<double>(size()) / static_cast<double>(CAPACITIES[mIndex_]);
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    std::vector<HashItem*> oldT = table_;
    table_.clear();
    table_.resize(CAPACITIES[mIndex_], nullptr);
		size_ = 0;
    for(typename std::vector<HashItem*>::const_iterator it = oldT.begin(); it != oldT.end(); it++)
    {
        if((*it) != nullptr && !(*it)->deleted)
        {
            HASH_INDEX_T loc = probe((*it)->item.first);
						table_[loc] = new HashItem((*it)->item);
						size_++;
        }
        delete *it;
    }
